...
from app.models import InterfaceTest,TestcaseResult,Interfacehuan,Project
from  app import db
...
class ApiTestCase():
    ...
    def testapi(self):
        for case in range(len(self.url)):
            ...
            if self.pid[case] !='' and self.pid[case] !='None' and self.pid[case] is not None:
                ...
                try:
                    ne = eval(self.headers[case])
                    api = Api(url=self.url[case], fangshi=self.meth[case], params=self.parm[case],
                              headers=ne)
                    apijson = api.getJson()
                    case_projects_id = testcase.projects_id
                    current_project = Project.query.filter_by(id=case_projects_id).first()
                    URL = Interfacehuan.query.filter_by(project=str(current_project.project_name)).first()
                    if self.saveresult[case] is True:
                        new_case = TestcaseResult(result=str(apijson), case_id=testcase.id,date=datetime.now(),testevir=URL.url)
